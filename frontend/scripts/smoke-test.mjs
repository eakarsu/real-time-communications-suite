const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:4700';
...
const adminCookie = await login('admin@real-time-comms.local', 'admin123');
const managerCookie = await login('manager@real-time-comms.local', 'manager123');
const analystCookie = await login('analyst@real-time-comms.local', 'analyst123');

await expectJson('/api/dashboard', adminCookie);
await expectJson('/api/entities/conversations', analystCookie);
await expectJson('/api/documents', analystCookie);
await expectStatus('/api/documents/upload', analystCookie, 405);

const records = await expectJson('/api/entities/conversations', managerCookie);
const rowId = records.rows[0].id;

const approveResponse = await fetch(`${baseUrl}/api/entities/conversations/approve`, {
  method: 'POST',
  headers: {
...
const forbiddenApprove = await fetch(`${baseUrl}/api/entities/conversations/approve`, {
  method: 'POST',
  headers: {
